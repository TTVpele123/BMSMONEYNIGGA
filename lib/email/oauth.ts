...
export function gmailAuthorizationUrl(state: string, mailbox: GmailOAuthMailbox = "send"): string {
  const legacy = mailbox === "legacy_inbound";
  return "https://accounts.google.com/o/oauth2/v2/auth?" + new URLSearchParams({
    client_id: process.env.GOOGLE_CLIENT_ID ?? "",
    redirect_uri: gmailRedirectUri(),
    response_type: "code",
    scope: (legacy ? GMAIL_LEGACY_INBOUND_SCOPES : GMAIL_SCOPES).join(" "),
    access_type: "offline",
    include_granted_scopes: legacy ? "false" : "true",
    prompt: "consent",
    login_hint: legacy ? PREVIOUS_SENDER : AUTHORIZED_SENDER,
    state,
  }).toString();
}
